...
public:
    ...
    const long long PENALTY_INFINITY = 1000000000;
...
int main() {
    ...
    if (env.loadFromFile(filename)) {
        ...
        std::cout << "Гібридна еволюція запущена (GPX3 + Mutation + 2-Opt)...\n";
        ...
        for (int gen = 1; gen <= generations; ++gen) {
            ga.evolve(gen);
            if (gen % 100 == 0) {
                std::cout << "Покоління " << gen << " | Відстань: " << ga.getBestIndividual().total_distance << "\n";
            }
        }
        ...
        std::cout << "\n============================================\n";
        std::cout << "ФІНАЛЬНИЙ ЗВІТ РОБОТИ АЛГОРИТМУ (МЕМІЧНИЙ ГА)\n";
        std::cout << "============================================\n";
        ...
        exportConvergenceToSVG("convergence_hybrid.svg", ga.convergence_history);
        exportRouteToSVG("route_hybrid.svg", best_final, env);
