...
public:
    ...
    StandardGeneticAlgorithm(int pop_size, const TSP_Environment& environment) 
    ...
    Individual orderCrossover(const Individual& parent1, const Individual& parent2) {
        ...
        int child_ptr = (idx2 + 1) % n;
        int parent_ptr = (idx2 + 1) % n;

        for (int count = 0; count < n; ++count) {
            int city = parent2.route[parent_ptr];
            if (!in_child[city]) {
                child.route[child_ptr] = city;
    ...
    void evolve(int current_generation) {
        ...
        while (new_pop.size() < population_size) {
            ...
            Individual child = orderCrossover(p1, p2);
            mutate(child);
...
void exportConvergenceToSVG(const std::string& filename, const std::vector<std::pair<int, long long>>& history) {
    ...
    file << "\" fill=\"none\" stroke=\"#dc3545\" stroke-width=\"2.5\" />\n";
...
int main() {
    ...
    TSP_Environment env;
    std::string filename = "16.txt"; 
    
    if (env.loadFromFile(filename)) {
        int population_size = 100;
        int generations = 1000; 

        StandardGeneticAlgorithm ga(population_size, env);
        ga.initializePopulation();
        
        std::cout << "Стандартна еволюція запущена (OX + Mutation)...\n";
        ...
        std::cout << "\n============================================\n";
        std::cout << "ЗВІТ: СТАНДАРТНИЙ ГЕНЕТИЧНИЙ АЛГОРИТМ (OX)\n";
        std::cout << "============================================\n";
